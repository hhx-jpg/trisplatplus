...
# Allow diagnosing long-running CUDA/PyTorch calls without a debugger.
...
class RenderCheckpointCallback(Callback):
    """Save lightweight encoder snapshots for asynchronous render exports.

    The regular Lightning checkpoints contain the frozen DA3 backbone and are
    several gigabytes each. Render exports reconstruct that base model, but
    must also carry any frozen task head that changes the primitive values
    (notably TSDPT ``gs_head`` during texture-only training).
    """

    def __init__(self, directory: Path, every_n_steps: int) -> None:
        super().__init__()
        self.directory = directory
        self.every_n_steps = max(int(every_n_steps), 1)
        self._last_saved_step: int | None = None

    def _save(self, trainer: Trainer, module: torch.nn.Module, step: int, file_name: str) -> None:
        ...
        # A render snapshot may omit the frozen DA3 backbone, depth head and
        # camera head because those are reconstructed from the base DA3
        # checkpoint.  The TSDPT ``gs_head`` is different: in texture-only
        # Stage A it is intentionally frozen, but it still defines opacity,
        # sigma, scale, SH and therefore every triangle's geometry.  Omitting
        # it makes the render monitor silently fall back to the untrained DA3
        # head, producing almost no visible triangles/black renders.
        def keep_for_render(name: str, parameter: torch.nn.Parameter) -> bool:
            return parameter.requires_grad or (
                name.startswith("encoder.da3.model.gs_head.")
                and "pretrained" not in name
            )

        state_dict = {
            parameter_name: parameter.detach().cpu()
            for parameter_name, parameter in module.named_parameters()
            if keep_for_render(parameter_name, parameter)
        }
        logger.info(
            "Render snapshot step=%d contains %d parameters (trainable=%d, frozen gs_head=%d).",
            step,
            len(state_dict),
            sum(parameter.requires_grad for parameter in module.parameters() if parameter.requires_grad),
            sum(
                name.startswith("encoder.da3.model.gs_head.")
                and "pretrained" not in name
        ...
        payload = {"state_dict": state_dict, "global_step": int(step)}
        temporary = path.with_name(path.name + ".tmp")
        torch.save(payload, temporary)
        temporary.replace(path)
        logger.info("Saved lightweight render checkpoint: %s", path)

    def on_fit_start(self, trainer: Trainer, pl_module: torch.nn.Module) -> None:
        # global_step is zero only for a fresh run (including a weights-only
        # initialization); a resumed training-state checkpoint keeps its step.
        if trainer.global_step == 0:
            self._save(trainer, pl_module, 0, "render_initial.ckpt")
    ...
    def on_train_batch_end(
        ...
        if step <= 0 or step % self.every_n_steps != 0 or step == self._last_saved_step:
            return
        self._save(trainer, pl_module, step, f"render_step_{step:06d}.ckpt")
        self._last_saved_step = step


class WeightOnlyResumeCallback(Callback):
    """Continue schedules and step accounting after loading model-only weights."""

    def __init__(self, resume_step: int) -> None:
        super().__init__()
        self.resume_step = max(int(resume_step), 0)
        self._applied = False
    ...
    def on_train_batch_start(
        ...
        if self._applied or self.resume_step <= 0:
            return

        # global_step is backed by this private Lightning loop counter. The
        # checkpoint has no optimizer state, so the normal checkpoint restore
        # path cannot populate it for us. This hook is deliberately delayed
        # until the first batch because Lightning resets the counter between
        # ``on_train_start`` and the epoch loop.
        manual_optimization = trainer.fit_loop.epoch_loop.manual_optimization
        manual_optimization.optim_step_progress.total.completed = self.resume_step
        manual_optimization.optim_step_progress.total.ready = self.resume_step
        manual_optimization.optim_step_progress.current.completed = 0
        manual_optimization.optim_step_progress.current.ready = 0

        # Put per-group LambdaLR schedulers at the same schedule position as
        # the saved model. Optimizer moments intentionally start fresh.
        for scheduler_config in getattr(trainer, "lr_scheduler_configs", []):
            scheduler_config.scheduler.step(self.resume_step)

        self._applied = True
        logger.info(
            "Loaded model-only checkpoint; continuing training at global step %d "
...
def train(cfg_dict: DictConfig):
    ...
    callbacks.append(
        RenderCheckpointCallback(
            output_dir / "checkpoints",
            cfg.checkpointing.every_n_train_steps,
        )
    )
    ...
    # Respect explicit Hydra trainer overrides (useful for single-scene
    # rendering) while preserving the historical automatic multi-GPU default.
    configured_devices = cfg_dict.trainer.get("devices")
    configured_strategy = cfg_dict.trainer.get("strategy")
    trainer_devices = "auto" if configured_devices is None else configured_devices
    ...
    elif str(configured_strategy).lower() == "ddp":
    ...
    weight_only_resume_step: int | None = None
        ...
        if checkpoint_has_training_state(checkpoint):
            ...
            try:
                ...
                if missing_keys or unexpected_keys:
                    ...
                    weight_only_resume_step = get_checkpoint_schedule_step(
    ...
    if cfg.mode == "train" and weight_only_resume_step is not None:
        trainer.callbacks.append(WeightOnlyResumeCallback(weight_only_resume_step))
