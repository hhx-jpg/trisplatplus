...
try:
    ...
    def _use_sdpa_backends(backends):
...
class FlashAttention(Attention):
    def forward(self, x: Tensor, attn_bias=None) -> Tensor:
        ...
        if q.dtype == torch.bfloat16:
            with _use_sdpa_backends(SDPBackend.FLASH_ATTENTION):
                x = scaled_dot_product_attention(q, k, v)
        else:
            with _use_sdpa_backends([SDPBackend.MATH, SDPBackend.EFFICIENT_ATTENTION]):
                x = scaled_dot_product_attention(q, k, v)
