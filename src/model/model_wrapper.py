...
class ModelWrapper(LightningModule):
    ...
    def _validate_primitives(self, primitives: Gaussians | Triangles) -> bool:
        if isinstance(primitives, Triangles):
            tensors = {
                ...
                "triangle.texture_colors": primitives.texture_colors,
                "triangle.texture_alphas": primitives.texture_alphas,
            }
        else:
            tensors = {
    ...
    def _apply_primitive_mask(
        ...
        return Triangles(
            ...
            texture_colors=primitives.texture_colors,
            texture_alphas=primitives.texture_alphas,
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        try:
            ...
            if self.global_rank == 0 and self.global_step % self.train_cfg.print_log_every_n_steps == 0:
                logger.info("train timing: encoder done; triangles=%s", getattr(primitives, "vertices", torch.empty(0)).shape)
            ...
            if not self._validate_primitives(primitives):
                ...
                if self.global_rank == 0 and self.global_step % self.train_cfg.print_log_every_n_steps == 0:
                    logger.info("train timing: decoder done")
        ...
        # Every DDP rank sees a different scene. Synchronize the skip decision
        # before backward so one pathological sample cannot update only a
        # subset of replicas and desynchronize their parameters.
        local_skip_sample = skip_sample
        if dist.is_available() and dist.is_initialized():
            skip_flag = torch.tensor(
                [int(local_skip_sample)],
                device=loss_device,
                dtype=torch.int64,
            )
            dist.all_reduce(skip_flag, op=dist.ReduceOp.MAX)
            skip_sample = bool(skip_flag.item())
            if skip_sample and not local_skip_sample:
                skip_reasons.append("large/non-finite loss on another rank")

        if skip_sample:
            total_loss = zero_loss
        logged_total_loss = total_loss
        ...
        if self.global_rank == 0 and self.global_step % self.train_cfg.print_log_every_n_steps == 0:
            logger.info("train timing: optimizer/backward done")
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Optional detached primitive export for downstream geometry experiments.
        # This is intentionally opt-in so normal evaluation artifacts are unchanged.
        dump_path = os.environ.get("TRISPLAT_DUMP_PRIMITIVES")
        if dump_path and batch_idx == 0 and self.global_rank == 0 and isinstance(primitives, Triangles):
            dump_file = Path(dump_path)
            dump_file.parent.mkdir(parents=True, exist_ok=True)
            torch.save(
                {
                    "vertices": primitives.vertices.detach().cpu(),
                    "features": primitives.features.detach().cpu(),
                    "opacity": primitives.opacity.detach().cpu(),
                    "sigma": primitives.sigma.detach().cpu(),
                    "scene": scene,
                    "context_indices": batch["context"]["index"].detach().cpu(),
                    "target_indices": batch["target"]["index"].detach().cpu(),
                    "context_extrinsics": batch["context"]["extrinsics"].detach().cpu(),
                    "context_intrinsics": batch["context"]["intrinsics"].detach().cpu(),
                    "context_near": batch["context"]["near"].detach().cpu(),
                    "context_far": batch["context"]["far"].detach().cpu(),
                    "context_image": batch["context"]["image"].detach().cpu(),
                    "target_extrinsics": batch["target"]["extrinsics"].detach().cpu(),
                    "target_intrinsics": batch["target"]["intrinsics"].detach().cpu(),
                    "target_near": batch["target"]["near"].detach().cpu(),
                    "target_far": batch["target"]["far"].detach().cpu(),
                    "target_image": batch["target"]["image"].detach().cpu(),
                },
                dump_file,
            )
            logger.info("Saved detached TriSplat primitives to %s", dump_file)
        ...
        for batch_idx in range(base_extrinsics.shape[0]):
            ...
            for view_idx in range(base_extrinsics.shape[1]):
                ...
                view_triangles = Triangles(
                    ...
                    texture_colors=triangle_batch.texture_colors,
                    texture_alphas=triangle_batch.texture_alphas,
                )
                view_outputs.append(
    ...
    def slice_triangle_batch(triangles: Triangles, batch_idx: int) -> Triangles:
        ...
        return Triangles(
            ...
            texture_colors=maybe_slice(triangles.texture_colors),
            texture_alphas=maybe_slice(triangles.texture_alphas),
        )
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        # Video encoding is opt-in; scalar metrics and comparison PNGs are the
        # validation artifacts used by the DDT smoke run.
        if getattr(self.test_cfg, "save_video", False):
    ...
    def configure_optimizers(self):
        ...
        opacity_params, opacity_param_names = [], []
        tsdpt_params, tsdpt_param_names = [], []
        pretrained_params, pretrained_param_names = [], []
        for name, param in self.named_parameters():
            if not param.requires_grad:
                continue

            if "gs_head.extra_output_conv.opacity." in name:
                opacity_params.append(param)
                opacity_param_names.append(name)
            elif "gs_head" in name:
                tsdpt_params.append(param)
                tsdpt_param_names.append(name)
            elif "texture_head" in name:
        ...
        param_dicts = []
        if new_params:
            param_dicts.append({"params": new_params, "lr": self.optimizer_cfg.lr})
        if opacity_params:
            param_dicts.append({
                "params": opacity_params,
                "lr": self.optimizer_cfg.opacity_lr
                if self.optimizer_cfg.opacity_lr is not None
                else self.optimizer_cfg.lr,
            })
        if tsdpt_params:
            param_dicts.append({
                "params": tsdpt_params,
                "lr": self.optimizer_cfg.tsdpt_lr
                if self.optimizer_cfg.tsdpt_lr is not None
        ...
        # SH-only ablations use gradient-masked rows of the shared GS output
        # projection. Disable decoupled weight decay so frozen non-SH rows do
        # not drift despite having a shared Parameter tensor.
        weight_decay = 0.0 if getattr(getattr(self.encoder, "cfg", None), "sh_only", False) else 0.05
        optimizer = torch.optim.AdamW(param_dicts, lr=self.optimizer_cfg.lr, weight_decay=weight_decay, betas=(0.9, 0.95))
        warm_up_steps = self.optimizer_cfg.warm_up_steps
        if self.optimizer_cfg.opacity_lr is None and self.optimizer_cfg.tsdpt_lr is None:
            # Preserve the native scheduler for experiments that do not use
            # the split TSDPT learning-rate groups.
            ...
            # LambdaLR is needed here because each group has a different base
            # rate; a scalar CosineAnnealingLR eta_min would be wrong for the
            # 1e-6 TSDPT group.
            warm_up_steps = max(int(warm_up_steps), 0)
            max_steps = max(int(get_cfg()["trainer"]["max_steps"]), warm_up_steps + 1)

            def lr_factor(step: int) -> float:
                if warm_up_steps > 0 and step < warm_up_steps:
                    return (1.0 / warm_up_steps) + (1.0 - 1.0 / warm_up_steps) * step / warm_up_steps
                progress = min(max((step - warm_up_steps) / (max_steps - warm_up_steps), 0.0), 1.0)
                return 0.1 + 0.45 * (1.0 + math.cos(math.pi * progress))

            lr_scheduler = torch.optim.lr_scheduler.LambdaLR(
                optimizer,
                lr_lambda=[lr_factor for _ in optimizer.param_groups],
            )
