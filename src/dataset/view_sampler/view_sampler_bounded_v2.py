...
class ViewSamplerBoundedV2(ViewSampler[ViewSamplerBoundedV2Cfg]):
    ...
    def sample(
        ...
        if self.cfg.fixed_context_indices is not None:
            context_views = torch.tensor(
                self.cfg.fixed_context_indices, dtype=torch.int64, device=device
            )
            target_views = torch.tensor(
                self.cfg.fixed_target_indices
                if self.cfg.fixed_target_indices is not None
                else self.cfg.fixed_context_indices,
                dtype=torch.int64,
                device=device,
            )
            if (
                context_views.numel() != self.num_context_views
                or target_views.numel() != self.cfg.num_target_views
                or context_views.min().item() < 0
                or target_views.min().item() < 0
                or context_views.max().item() >= num_views
                or target_views.max().item() >= num_views
            ):
                raise ValueError(
                    "Fixed view indices must match the configured view counts "
                    f"and lie in [0, {num_views}); got context={context_views.tolist()} "
                    f"target={target_views.tolist()}"
                )
            return (
                context_views,
                target_views,
