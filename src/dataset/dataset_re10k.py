...
class DatasetRE10k(IterableDataset):
    ...
    def __init__(
        ...
        for root in roots:
            ...
            chunk_root = stage_root if stage_root.exists() else root
            # Some packed DL3DV mirrors only provide a train chunk directory,
            # while a test-time overfit/evaluation request still needs to load
            # a scene from that split. Reuse the train chunks when no test
            # directory exists instead of silently creating an empty loader.
            if (
                self.stage == "test"
                and not stage_root.exists()
                and (root / "train").is_dir()
            ):
                chunk_root = root / "train"
            root_chunks = sorted(
                [path for path in chunk_root.iterdir() if path.suffix == ".torch"]
            )
        ...
        if self.cfg.overfit_to_scene is not None:
            ...
            # Keep at least one iteration when the requested scene lives in a
            # train-only packed split and the test directory has no chunks.
            self.chunks = [chunk_path] * max(len(self.chunks), 1)
    ...
    def __iter__(self):
        ...
        for chunk_path in self.chunks:
            ...
            for example in chunk:
                ...
                # Preserve native pixels and intrinsics for appearance heads.
                # The regular ``image`` field is cropped to the DA3 geometry
                # resolution below; texture prediction must retain information
                # that would otherwise be irreversibly discarded.
                example["context"]["image_highres"] = context_images.clone()
                example["target"]["image_highres"] = target_images.clone()
                example["context"]["intrinsics_highres"] = intrinsics[context_indices].clone()
                example["target"]["intrinsics_highres"] = intrinsics[target_indices].clone()
    ...
    def index(self) -> dict[str, Path]:
        merged_index = {}
        loaded_index_paths: set[Path] = set()
        data_stages = [self.data_stage]
        if self.cfg.overfit_to_scene is not None:
            data_stages = ("test", "train")
        for data_stage in data_stages:
            for root in self.cfg.roots:
                # Load the root's index.  Packed DL3DV roots may store the
                # split index one directory above ``10K/train`` while the
                # chunks themselves live below the split directory.
                stage_root = root / data_stage
                index_path = None
                index_base = None
                # Prefer the stage-local index, then walk upward for packed
                # layouts such as <dataset>/10K/train/*.torch with a single
                # <dataset>/index.json.
                for candidate_base in (stage_root, *stage_root.parents):
                    candidate_path = candidate_base / "index.json"
                    if candidate_path.is_file():
                        index_path = candidate_path
                        index_base = candidate_base
                        break
                if index_path is None or index_base is None:
                    continue
                index_path = index_path.resolve()
                if index_path in loaded_index_paths:
                    continue
                loaded_index_paths.add(index_path)
                with index_path.open("r") as f:
                    index = json.load(f)
                index = {k: Path(index_base / v) for k, v in index.items()}
