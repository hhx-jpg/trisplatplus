...
def main() -> None:
    ...
    # Keep the native TriSplat primitive attributes for downstream RDEL/mesh
    # experiments; the direct PLY export loses per-triangle opacity and sigma.
    torch.save(
        {
            ...
            "scene": scene,
            "context_indices": torch.tensor([selected_indices]),
            "context_extrinsics": visualization_dump.get("c2w", mesh_batch["context"]["extrinsics"]).detach().cpu(),
            "context_intrinsics": intrinsics.unsqueeze(0),
            "context_image": images.unsqueeze(0),
        },
        out_dir / "trisplat_primitives.pt",
    )
