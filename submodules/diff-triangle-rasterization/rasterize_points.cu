...
RasterizetrianglesCUDA(
	...
	const bool debug,
	const torch::Tensor& texture_colors,
	...
	const float texture_color_sigma)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		tan_fovy,
		prefiltered,
		texture_colors.contiguous().data_ptr<float>(),
		texture_size,
		texture_color_sigma,
	...
	const float texture_color_sigma)
{
  ...
  torch::Tensor dL_dtexture_colors = texture_size > 0
      ? torch::zeros_like(texture_colors)
      : torch::empty({0}, triangles_points.options());
  ...
  if(P != 0)
  {
	  ...
	  texture_size,
	  texture_color_sigma,
	  dL_dtexture_colors.contiguous().data_ptr<float>());
  }

  return std::make_tuple(dL_dtriangle, dL_dsigma, dL_dcolors, dL_dopacity, dL_dsh, dL_dmeans2D, dL_dtexture_colors);
}
