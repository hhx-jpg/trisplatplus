...
// Backward pass of the preprocessing steps, except
 ...
 renderCUDA(
	 ...
	 float* __restrict__ dL_dtexture_colors)
 {
	 ...
	 const float2 pixf = mesh_coverage
	 ...
	 */
	 ...
	 __shared__ float2 collected_projected_points[BLOCK_SIZE * MAX_NB_POINTS];
	 /*
	 ===================================================================================================
	 */
	...
	float last_dL_dT = 0;
	 ...
	 for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	 {
		 ...
		 if (range.x + progress < range.y)
		 {
			 ...
			 for (int k = 0; k < 3; k++) {
				...
				collected_offsets[MAX_NB_POINTS * block.thread_rank() + k] = offsets[cumsum_of_points_per_triangle[coll_id] + k];
				 ...
				 for (int k = 0; k < 3; ++k)
					 collected_projected_points[MAX_NB_POINTS * block.thread_rank() + k] =
						 projected_points[cumsum_of_points_per_triangle[coll_id] + k];
		 }
		 block.sync();
		 ...
		 for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		 {
			 ...
			 const float alpha = mesh_coverage ? 0.99f : min(0.99f, con_o.w * Cx);
			 ...
			 const int global_id = collected_id[j];
				 float3 sampled_texture = make_float3(0.0f, 0.0f, 0.0f);
				 if (texture_colors != nullptr && texture_size > 0) {
					 sampled_texture = sampleTriangleTexture(
						 texture_colors, global_id, pixf,
						 collected_projected_points + base,
						 0, texture_size, texture_color_sigma);
				 }
				 for (int ch = 0; ch < C; ch++)
				 {
					 const float c = (texture_colors != nullptr && texture_size > 0)
						 ? (ch == 0 ? sampled_texture.x : (ch == 1 ? sampled_texture.y : sampled_texture.z))
						 : collected_colors[ch * BLOCK_SIZE + j];
				 ...
				 // many that were affected by this Triangle.
					 if (texture_colors != nullptr && texture_size > 0) {
						 float texture_grad[3] = {0.0f, 0.0f, 0.0f};
						 texture_grad[ch] = dchannel_dcolor * dL_dchannel;
						 accumulateTriangleTextureGrad(
							 dL_dtexture_colors, global_id, pixf,
							 collected_projected_points + base, 0,
							 texture_size, texture_color_sigma, texture_grad);
					 } else {
			 ...
			 if (!mesh_coverage && phi_final > 0.0f) {
			...
			// Calculate gradient w.r.t phi_x
			 float dL_dphi_x = mesh_coverage
				 ? 0.0f
				 : dL_dC * (sigma_pre / phi_x) * Cx;
