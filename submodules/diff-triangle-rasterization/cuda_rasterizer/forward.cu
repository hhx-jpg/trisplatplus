...
#ifndef TRI_DISABLE_OPACITY_CULL
#define TRI_DISABLE_OPACITY_CULL 0
#endif
	 ...
	 bool mesh_coverage)
 {
	 ...
	 // The threshold is retained by default and can be disabled for culling
	 // ablations at compile time.
#if !TRI_DISABLE_OPACITY_CULL
	 ...
	 for (int i = 0; i < 3; i++) {
		...
		if (i != 0){
			...
			if (fabsf(det) < 1e-3) {
				...
				if (mesh_coverage) {
					bx0 = clampTileBound(intersect_x, BLOCK_X, grid.x, false);
					by0 = clampTileBound(intersect_y, BLOCK_Y, grid.y, false);
					bx1 = clampTileBound(intersect_x, BLOCK_X, grid.x, true);
					by1 = clampTileBound(intersect_y, BLOCK_Y, grid.y, true);
				} else {
					bx0 = min(grid.x, max(0, (uint)(intersect_x / BLOCK_X)));
					by0 = min(grid.y, max(0, (uint)(intersect_y / BLOCK_Y)));
					bx1 = min(grid.x, max(0, (uint)((intersect_x + BLOCK_X - 1) / BLOCK_X)));
					by1 = min(grid.y, max(0, (uint)((intersect_y + BLOCK_Y - 1) / BLOCK_Y)));
				}
	...
	// Preserve the native ordinary triangle-splat path, but do not discard
	// sub-pixel projected faces.  The old one-pixel inradius gate was intended
	// for Gaussian-like splats; for the DA3 triangle chart it creates large
	// holes because most valid local faces have an inradius below one pixel.
	// Use the same tiny numerical guard as mesh_coverage and let the
	// pixel-center coverage test decide whether a face contributes.
	 constexpr float distance_eps = 1e-4f;
	 constexpr float incenter_eps = 1e-6f;
	 const bool invalid_triangle =
		 distance_points > 1600 or distance_points < distance_eps or dist > -incenter_eps;
	 if (invalid_triangle) {
		 radii[idx] = 0;
		 tiles_touched[idx] = 0;
		 scaling[idx] = 0.0f;
	...
	// The mesh path tests exact triangle coverage at pixel centers.  Keep a
	// one-tile guard band around the projected triangle so small numerical
	// reprojection errors cannot prevent a tile from receiving the face.
	if (mesh_coverage && rect_max_triangle_test.x > rect_min_triangle_test.x &&
		rect_max_triangle_test.y > rect_min_triangle_test.y) {
		rect_min_triangle_test.x = rect_min_triangle_test.x > 0
			? rect_min_triangle_test.x - 1 : 0;
		rect_min_triangle_test.y = rect_min_triangle_test.y > 0
			? rect_min_triangle_test.y - 1 : 0;
		rect_max_triangle_test.x = min(grid.x, rect_max_triangle_test.x + 1);
		rect_max_triangle_test.y = min(grid.y, rect_max_triangle_test.y + 1);
	}

	 if (mesh_coverage &&
		 (rect_max_triangle_test.x <= rect_min_triangle_test.x ||
		  rect_max_triangle_test.y <= rect_min_triangle_test.y)){
		 // A sub-pixel triangle can legitimately fit inside one tile.  Submit
		 // that tile instead of treating a zero tile area as an invalid face.
		 // Clamp the center tile because projected centers may lie outside the
		 // image while their triangle still intersects an image tile.
		 int center_tile_x = (int)floorf(center_triangle_2D.x / BLOCK_X);
		 int center_tile_y = (int)floorf(center_triangle_2D.y / BLOCK_Y);
		 center_tile_x = max(0, min((int)grid.x - 1, center_tile_x));
		 center_tile_y = max(0, min((int)grid.y - 1, center_tile_y));
		 rect_min_triangle_test = {(uint)center_tile_x, (uint)center_tile_y};
		rect_max_triangle_test = {(uint)(center_tile_x + 1), (uint)(center_tile_y + 1)};
	 }
	 if (!mesh_coverage &&
		 (rect_max_triangle_test.x - rect_min_triangle_test.x) *
		 (rect_max_triangle_test.y - rect_min_triangle_test.y) == 0){
		radii[idx] = 0;
		tiles_touched[idx] = 0;
		scaling[idx] = 0.0f;
		return;
	...
	// Store the final (possibly one-tile fallback) rectangle.  These values
	// are consumed by duplicateWithKeys together with tiles_touched, so they
	// must be written after the fallback is applied.
 ...
 renderCUDA(
	 ...
	 const float2* __restrict__ projected_points,
	 ...
	 const float* __restrict__ texture_colors,
	 ...
	 float* __restrict__ max_blending)
 {
	 ...
	 // Mesh faces connect projected pixel samples. Sampling at integer
	 // coordinates puts one-pixel cells on shared vertices/edges and makes
	 // their coverage zero. A negative texture sigma is an internal marker
	 // for the indexed-mesh coverage path.
	 const bool mesh_coverage = texture_color_sigma < 0.0f;
	 float2 pixf = mesh_coverage
		 ? make_float2((float)pix.x + 0.5f, (float)pix.y + 0.5f)
		 : make_float2((float)pix.x, (float)pix.y);
		 ...
		 for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		 {
			 ...
			 for (int k = 0; k < 3; k++) {
				 ...
				 float dist = (collected_normals[base + k].x * pixf.x
					 ...
					 if (dist > (mesh_coverage ? 1e-5f : 0.0f)) {
					outside = true;
					break;
				 }
			 ...
			 float Cx = mesh_coverage
				 ? 1.0f
				 : fmaxf(0.0f, __powf(phi_final, sigma_pre));


			 // An indexed mesh is an opaque surface.  Using the source Gaussian
			 // opacity here makes two coplanar faces alpha-blend on every shared
			 // diagonal, producing the visible brown seams.  Keep a tiny
			 // transmittance margin because the compositing code expects alpha<1.
			 float alpha = mesh_coverage ? 0.99f : min(0.99f, con_o.w * Cx);
			 if ((!mesh_coverage && alpha < 1.0f / 255.0f)
				 || (mesh_coverage && alpha < 1e-6f))
				 continue;
			 ...
			 for (int ch=0; ch<3; ch++) N[ch] += normal[ch] * blending_weight;
				 ...
				 if (texture_colors != nullptr && texture_size > 0) {
					 float3 tex = sampleTriangleTexture(
						 texture_colors, j_id, pixf, projected_points,
						 cumsum_of_points_per_triangle[j_id], texture_size,
						 texture_color_sigma);
					 C[0] += tex.x * alpha * T;
					 C[1] += tex.y * alpha * T;
					 C[2] += tex.z * alpha * T;
