...
class _RasterizeTriangles(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(triangles_points, sigma, num_points_per_triangle, cumsum_of_points_per_triangle, colors_precomp, radii, sh, geomBuffer, binningBuffer, imgBuffer, texture_colors)
    ...
    def backward(ctx, grad_out_color, _, __, ___, grad_depth, _____):
        ...
        triangles_points, sigma, num_points_per_triangle, cumsum_of_points_per_triangle, colors_precomp, radii, sh, geomBuffer, binningBuffer, imgBuffer, texture_colors = ctx.saved_tensors
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args) # Copy them before they can be corrupted
            try:
                grad_triangles, grad_sigma, grad_colors_precomp, grad_opacities, grad_sh, grad_means2D, grad_texture_colors = _C.rasterize_triangles_backward(*args)
            except Exception as ex:
...
class TriangleRasterizer(nn.Module):
    ...
    def forward(self, triangles_points, sigma, num_points_per_triangle, cumsum_of_points_per_triangle, number_of_points, opacities, means2D, scaling, density_factor,  shs = None, colors_precomp = None, texture_colors = None, texture_size = 0, texture_color_sigma = 1.0):
        ...
        if texture_colors is None:
            texture_colors = _empty_like_reference(triangles_points)
            texture_size = 0
        else:
            _require_cuda_tensor("texture_colors", texture_colors)
            if texture_colors.ndim != 4 or texture_colors.shape[0] != number_of_points or texture_colors.shape[1] != 3:
                raise ValueError("texture_colors must have shape [number_of_points, 3, texture_size, texture_size]")
            if texture_colors.shape[2] != texture_size or texture_colors.shape[3] != texture_size:
                raise ValueError("texture_colors spatial dimensions must match texture_size")
            if texture_size <= 0:
                raise ValueError("texture_size must be positive when texture_colors are provided")
