...
class DepthAnything3Net(nn.Module):
    ...
    def _process_gs_head(
        ...
        extrinsics: torch.Tensor | None = None,
        intrinsics: torch.Tensor | None = None,
        build_gaussians: bool = True,
    ) -> Dict[str, torch.Tensor]:
        """Process GS/TSDPT outputs, optionally converting them to Gaussians.

        Triangle consumers need the per-pixel GS/TSDPT head outputs but build
        their own geometry from a point map.  ``build_gaussians=False`` keeps
        that path from doing the expensive native GaussianAdapter conversion.
        The default preserves the original DA3 behavior.
        """
        if self.gs_head is None or (build_gaussians and self.gs_adapter is None):
            return output
        ...
        output["raw_gs"] = raw_gaussians
        output["raw_gs_conf"] = densities
        # TSDPT exposes triangle-specific attributes in addition to the
        # checkpoint-compatible Gaussian tensors. Keep them on the public
        # output so a TriSplat adapter can consume them without re-running
        # the frozen DA3 backbone/head.
        for name in ("raw_gs_opacity", "raw_gs_sigma", "raw_gs_scale"):
            if hasattr(gs_outs, name):
                output[name] = getattr(gs_outs, name)

        if not build_gaussians:
            return output
